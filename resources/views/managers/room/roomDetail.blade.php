        ...
        <div class="row mb-5">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="room_type_id" name="room_type_id" required>
                            @foreach($roomTypes as $type)
                                <option value="{{ $type->id }}" {{ $room->roomType->id == $type->id ? 'selected' : '' }}>
                                    {{ $type->name }}</option>
                            @endforeach
                        </select>
                    </div>
            ...
            <div class="col-md-6">
                ...
                <p><strong>Status:</strong>
                    {{ $room->status == 0 ? 'Available' : ($room->status == 1 ? 'Occupied' : 'Unavailable') }}</p>
        ...
        <div class="row">
            ...
            <div class="col-4">
                ...
                <button class="btn btn-outline-secondary {{ $room->status == "0" ? 'text-bg-primary' : ''}}">
                    <a href="{{ route('managers.updateRoomStatus', ['id' => $room->id, 'status' => 0]) }}"
